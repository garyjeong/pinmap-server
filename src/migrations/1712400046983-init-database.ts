...
export class InitDatabase1712400046983 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`user\` (\`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL, \`id\` int NOT NULL AUTO_INCREMENT, \`email\` varchar(20) NOT NULL COMMENT '이메일', \`password\` varchar(255) NULL COMMENT '비밀번호, SNS의 경우 Null', \`username\` varchar(255) NULL COMMENT '닉네임', UNIQUE INDEX \`IDX_e12875dfb3b1d92d7d7c5377e2\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB COMMENT="사용자 테이블"`);
        ...
        await queryRunner.query(`CREATE TABLE \`user_group\` (\`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL, \`id\` int NOT NULL AUTO_INCREMENT, \`is_owner\` tinyint NOT NULL COMMENT '그룹장 여부', \`usersId\` int NULL, \`groupId\` varchar(36) NULL, \`statusId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB COMMENT="사용자와 그룹 중간 테이블"`);
        await queryRunner.query(`CREATE TABLE \`group\` (\`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL, \`id\` varchar(36) NOT NULL, \`name\` varchar(20) NOT NULL COMMENT '그룹 이름', PRIMARY KEY (\`id\`)) ENGINE=InnoDB COMMENT="사용자 그룹 테이블"`);
